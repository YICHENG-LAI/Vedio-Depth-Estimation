...
function pairwise = pairwise_structure(img_left,Ws,epsilon)
...
for row = 0:H-1
  for col = 0:W-1
    ...
    if row+1 < H
        ...
        d = dist(img_left(row+1,col+1,:),img_left(row+2,col+1,:));
    ...
    if row-1 >= 0
        pairwise_x(end + 1) = pixel; 
        pairwise_y(end + 1) = 1+col+(row-1)*W;
        d = dist(img_left(row+1,col+1,:),img_left(row,col+1,:));
        Ux_tmpt(end+1) = 1 / (d + epsilon);
        Nx = Nx + 1;
    end 
    if col+1 < W
        ...
        d = dist(img_left(row+1,col+1,:),img_left(row+1,col+2,:));
        Ux_tmpt(end+1) = 1 / (d + epsilon);
        Nx = Nx + 1;
    end
    if col-1 >= 0
        pairwise_x(end + 1) = pixel; 
        pairwise_y(end + 1) = 1+(col-1)+row*W;
        d = dist(img_left(row+1,col+1,:),img_left(row+1,col,:));
        Ux_tmpt(end+1) = 1 / (d + epsilon);
        Nx = Nx + 1;
    end
    Ux = Nx / sum(Ux_tmpt);
